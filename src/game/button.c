...
Button buttonCreate(const char* text, int x, int y, int w, int h) {
    TraceLog(LOG_INFO, "Creating button");
...
void buttonDraw(Button* b) {
    ...
    DrawTextEx(font, b->text, (Vector2){textX, textY}, b->fontSize, 1.0f,
               BLACK);
}
 ...
 * @return A boolen, true if the Button was pressed, false if the Button was not
 * pressed
 */
