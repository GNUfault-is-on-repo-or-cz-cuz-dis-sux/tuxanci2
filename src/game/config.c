...
char* configRead(const char* entry) {
    TraceLog(LOG_INFO, "Reading from config");
...
void configWrite(const char* entry, const char* data) {
    TraceLog(LOG_INFO, "Writing to config");
...
void configDefaults(void) {
    TraceLog(LOG_INFO, "Writing defaults to config");
    configWrite("MUTE", "false");
}

/// @brief Init the config path 
void configInit(void) {
    TraceLog(LOG_INFO, "Initializing config");
...
void configDestroy(void) {
    TraceLog(LOG_INFO, "Cleaning up config stuff");
    free(configFile);
}
